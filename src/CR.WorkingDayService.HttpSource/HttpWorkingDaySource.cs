...
namespace CR.WorkingDayService.HttpSource
{
    ...
    /// <summary>
    /// An implementation of <see cref="StringWorkingDaySource{T}"/> which uses the returned content of a HTTP Endpoint as the source for determining if a <see cref="DateTime"/> is on a Working Day.
    /// </summary>
    public sealed class HttpWorkingDaySource<T> : StringWorkingDaySource<T>, IDisposable
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="HttpWorkingDaySource{T}"/> class which builds a state based on the content returned by the provided <see cref="HttpRequestMessage"/> which is parsed via the provided <c>parseAction</c>.
        /// The built internal state can be used to determine if a given <see cref="DateTime"/> is on a Working Day or Non-Working Day.
        /// </summary>
        /// <param name="request">The HTTP Request to make who's repsonse will be used to build the <see cref="HttpWorkingDaySource{T}"/>'s internal state.</param>
        /// <param name="parseAction">The Action which the response from the HTTP endpoint will be passed to, in order to build the<see cref="HttpWorkingDaySource{T}"/>'s internal state.</param>
        /// <param name="checkAction">The Action used to to determine if a given <see cref="DateTime"/> is a Working Day or a Non-Working Day.</param>
