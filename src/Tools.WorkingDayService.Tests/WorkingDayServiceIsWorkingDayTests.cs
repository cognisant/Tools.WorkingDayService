...
namespace CR.Tools.WorkingDayService.Tests
{
    ...
    public class WorkingDayServiceIsWorkingDayTests
    {
        ...
        /// </summary>
        [Test]
        public static void WorkingDayServiceWithNoSourcesThrowsArgumentException() => Assert.Throws<ArgumentException>(() => { WorkingDayService _ = WorkingDayServiceBuilder.New(); });

        /// <summary>
        /// Test to check that a Working Day Service with one Source considers a Working Day a Working Day.
        /// </summary>
        [Test]
        public static void WorkingDayServiceWithOneSourceReturnsTrueWhenIsWorkingDayIsCalledOnAWorkingDay() => Assert.IsTrue(((WorkingDayService)WorkingDayServiceBuilder.New().AddDaysOfTheWeekNonWorkingDaySource(DayOfWeek.Monday)).IsWorkingDay(new DateTime(2018, 5, 15)));

        /// <summary>
        /// Test to check that a Working Day Service with one Source considers a non-Working Day a non-Working Day.
        /// </summary>
        [Test]
        public static void WorkingDayServiceWithOneSourceReturnsFalseWhenIsWorkingDayIsCalledOnANonWorkingDay() => Assert.IsTrue(((WorkingDayService)WorkingDayServiceBuilder.New().AddDaysOfTheWeekNonWorkingDaySource(DayOfWeek.Monday)).IsNonWorkingDay(new DateTime(2018, 5, 14)));
